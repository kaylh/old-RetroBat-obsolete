...
set "emulator_dir=%retroarch_main_dir%\emulators"
set "decorations_dir="%retroarch_main_dir%\system\decorations"
REM END PATH
...
if "!update_emulationstation!"=="1" (
	...
	if "%enable_download%"=="1" "%modules_dir%\rb_updater\wget" --no-check-certificate wget --retry-connrefused --waitretry=1 --read-timeout=20 --timeout=15 -t %download_retry% -P "%download_dir%" https://www.retrobat.ovh/repo/win64/%banch%/%package_name% -q >nul	
	...
	if "%enable_extraction%"=="1" "%modules_dir%\rb_updater\7za.exe" -y x "%download_dir%\%package_name%" -aoa -o"%extraction_dir%\emulationstation" >nul	
	cls
	set /a progress_percent=100*!progress_current!/progress_total
	echo updating retrobat ^>^>^> !progress_percent!%%	
	timeout /t 1 >nul
...
if "%update_theme_carbon%"=="1" (
	...
	if "%enable_extraction%"=="1" if exist "%default_theme_dir%\es-theme-carbon_old\." rmdir /s /q "%default_theme_dir%\es-theme-carbon_old\"
	if "%enable_extraction%"=="1" if exist "%default_theme_dir%\es-theme-carbon\." move "%default_theme_dir%\es-theme-carbon" "%default_theme_dir%\es-theme-carbon_old"	
...
if "%update_retrobat_decorations%"=="1" (
	...
	if "%enable_extraction%"=="1" "%modules_dir%\rb_updater\7za.exe" -y x "%download_dir%\decorations.7z" -aoa -o"%decorations_dir%" >nul	
	cls
	set /a progress_percent=100*!progress_current!/progress_total
	echo updating retrobat ^>^>^> !progress_percent!%%
...
for /f "delims=:: tokens=*" %%a in ('findstr /b :: "%~f0"') do (
	...
	if "!update_%%a!"=="1" if not exist "%emulator_path%\%%a\manual_update.txt" (
	REM DOWNLOAD
...
if "%update_retrobat_main%"=="1" (
	...
	if "%update_es_settings%"=="1" if exist "%emulationstation_dir%\.emulationstation\es_settings.cfg" (
		copy/y "%emulationstation_dir%\.emulationstation\es_settings.cfg" "%emulationstation_dir%\.emulationstation\es_settings.cfg.old"
		copy/y "%emulationstation_dir%\..\system\templates\emulationstation\es_settings.cfg" "%emulationstation_dir%\.emulationstation\es_settings.cfg"
	) >nul
	
	if "%update_es_systems%"=="1" if exist "%emulationstation_dir%\.emulationstation\es_system.cfg" (
		copy/y "%emulationstation_dir%\.emulationstation\es_system.cfg" "%emulationstation_dir%\.emulationstation\es_system.cfg.old"
		copy/y "%emulationstation_dir%\..\system\templates\emulationstation\es_system.cfg" "%emulationstation_dir%\.emulationstation\es_system.cfg"
	) >nul
	
	if "%update_es_features%"=="1" if exist "%emulationstation_dir%\.emulationstation\es_features.cfg" (
		copy/y "%emulationstation_dir%\.emulationstation\es_features.cfg" "%emulationstation_dir%\.emulationstation\es_features.cfg.old"
		copy/y "%emulationstation_dir%\..\system\templates\emulationstation\es_features.cfg" "%emulationstation_dir%\.emulationstation\es_features.cfg"
	) >nul
	
	if "%update_es_padtokey%"=="1" if exist "%emulationstation_dir%\.emulationstation\es_padtokey.cfg" (
		copy/y "%emulationstation_dir%\.emulationstation\es_padtokey.cfg" "%emulationstation_dir%\.emulationstation\es_padtokey.cfg.old"
		copy/y "%emulationstation_dir%\..\system\templates\emulationstation\es_padtokey.cfg" "%emulationstation_dir%\.emulationstation\es_padtokey.cfg"
	) >nul
